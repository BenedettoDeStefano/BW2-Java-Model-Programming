...
public class RivenditoreAutorizzatoDAO {
	...
	public RivenditoreAutorizzato getRivenditoreAutorizzatoById(long l) {
		try {
			return em.find(RivenditoreAutorizzato.class, l);
		} catch (Exception e) {
			log.error("Errore durante il recupero del rivenditore autorizzato con ID " + l + ": " + e.getMessage());
			return null;
		}
	}
